...
class SexPredDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        victim_padding = self.__max_length - len(input_tensor)-1
        perv_padding = self.__max_length - len(output_tensor)-1
        ...
        return torch.cat((torch.tensor([1]),input_tensor)),torch.cat((torch.tensor([1]),output_tensor))